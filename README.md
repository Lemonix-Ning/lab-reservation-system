...
-  **日历调度**：可视化日/周/月视图，多维度资源调度，实时冲突检测
-  **智能推荐**：预约冲突时自动推荐替代时间段，支持一键应用并重新提交审核
-  **热力图视图**：月度资源利用率可视化，直观展示资源使用情况
-  **移动端优化**：响应式设计，触摸友好的操作界面
-  **数据导出**：支持导出为 HTML / iCalendar / PDF 格式，可直接查看或导入外部日历
...
#### 日历视图 - 智能替代方案推荐（2025-12-12 新增）

1. 进入 **日历调度** (`/calendar`)
2. 修改预约时间导致冲突时，系统自动检测
3. 点击 **查看替代方案** 按钮
4. AI 智能分析算法推荐最多5个替代时间段：
   - **同日调整**：在同一天±1-12小时寻找空闲时段
   - **未来同期**：后续21天的相同时间段
   - **间隙分析**：分析现有预约间的可用时间
   - **置信度评分**：根据时间接近度和日期早晚打分
5. 一键应用推荐方案，自动重新提交审核

**算法特点**：
- 智能过滤：仅推荐工作时间（6:00-22:00）
- 规则检查：自动符合ADVANCE_DAYS规则
- 冲突避让：确保推荐时段无冲突
- 优先排序：越接近原时间、越早的日期排名越高
...
**最后更新**：2025年12月21日  
**项目版本**：1.5.0 (P2-1 日历与可视化调度完成)  
**开发状态**：P2-1 日历与可视化调度模块已完成，包括日/周/月/热力图视图、移动端响应式优化、HTML 导出功能、预约详情模态框展示。所有功能已通过测试验证。待完成：P2-2 AI 智能增强、Phase 4 签到与定位对账。
